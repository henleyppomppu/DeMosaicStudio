...
.\.venv\Scripts\python.exe -m pytest        # 현재 300 통과 · 2 스킵 (약 48초)
...
**"통과"를 "검증됨"으로 착각하지 마세요.** 402개(C# 102 + Python 300) 중 실제 미디어·GPU를
...
2. **미디어 계층 룩어헤드.** 지금 창은 §5.6이 규정한 중앙 정렬이 아니라 **인과적(causal)**
...
3. **전 구간 품질 검사를 테스트로.** 지금은 `scripts/eval_endtoend.py`라는 손으로 돌리는
   스크립트입니다. 회귀를 잡아야 할 숫자가 CI 밖에 있습니다.
4. **수집해야만 하는 네거티브.** §11.4의 절반(저비트레이트 블로킹·리샘플링·그레인)은 합성으로
   만들었고 **그 절반은 이미 다 썼습니다** — v0.2.0이 영상 단위 오발 18.8%→9.4%로 줄인 대가가
   IoU 0.032였습니다. 남은 것은 만들 수 없는 것들입니다: 진짜 디포커스/보케, 픽셀아트,
   LED월, 메시 원단.
5. **두 번째 CC 소스.** 지금 수치는 전부 영화 한 편이라 콘텐츠 간 일반화를 보지 못합니다.
6. **처리량.** 실측 0.63 fps 대 §6.1의 MVP 목표 ≥4 fps. **6배 이상 모자랍니다.** 품질이
   먼저지만, 이 격차는 설계 문제이지 튜닝 문제일 가능성이 높으므로 기록해 둡니다.
7. **학습된 복원기 — 여전히 가장 중요한 미해결 질문.** 지금까지의 수치는 전부 **고전
   해법(IBP)** 입니다. IBP는 정렬된 픽셀을 전부 증거로 취급합니다. 학습된 복원기는 대응이 나쁜
   증거를 무시하도록 학습될 수 있고, 그게 되는지가 다중 프레임이 slow 대역 밖에서도 살아남는지를
   결정합니다.
8. **LPIPS가 생기면 게이트를 다시 돌리세요.** §1.4.3은 PSNR·LPIPS·워핑을 함께 보라고 하는데
   셋 중 하나만 쟀습니다.
9. 곁들여: `Speed` 인코더 프로파일이 `tools/ffmpeg`를 서브프로세스로 불러야 합니다 (§4 발견 7).
