...
dotnet test  DeMosaicStudio.slnx -c Release   # 현재 102/102
.\.venv\Scripts\python.exe -m pytest        # 현재 323 통과 · 2 스킵 (약 57초)
...
**"통과"를 "검증됨"으로 착각하지 마세요.** 425개(C# 102 + Python 323) 중 실제 미디어·GPU를
...
1. **출력을 눈으로 보세요.** 지금까지 **아무도 어떤 출력도 본 적이 없습니다.** −0.69 dB는
   보기에 더 나을 수도 더 나쁠 수도 있고, 복원에서 PSNR은 약한 대리 지표입니다. 이건 몇 분이면
   되는 일이고, 아래 어느 항목보다 값싸게 방향을 바꿀 수 있습니다.
   `artifacts/ladder_*.mp4`가 그대로 있습니다.
2. **개선하지 못할 픽셀은 쓰지 않게 하기.** 분해가 가리키는 곳입니다 — 복원이 닿는 곳마다
   악화시키는데(안 −0.69, 밖 −4.21 dB) 그걸 막는 것이 아무것도 없습니다.
   `minRestorationConfidence`는 존재하지만 기본값이 0.0, 즉 꺼져 있습니다. 이건 모델이 아니라
   게이트라 재기 싸고, 래더로 바로 검증됩니다. **스마트컷(R-1.8c)은 아닙니다** — 상한이 0인 것을
   쟀습니다.
