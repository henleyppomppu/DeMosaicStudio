...
**Version:** 3.3 — dense alignment measured; multi-frame's operating window narrowed to what the data supports (supersedes 3.2)
**Distribution:** **Personal use only. This software is not distributed to anyone.** See §2.4 and D-11.
**Version history:** `prd.v1.0-original.md` (original), `prd.v2.0.md` (implementation-ready), `prd.v3.0.md` (personal-use scope), `prd.v3.1.md` (control gaps closed), `prd.v3.2.md` (Phase 0 gate)
...
## Revision Note — what v3.3 changes and why

**Dense optical flow was implemented and measured** (`docs/phase2-alignment-report.md`). It closes
19% of the alignment gap v3.2 called Phase 2's critical path, and the measurement redirects the plan
again:

1. **§18 R-13 was half right.** Alignment was worth fixing — residual after alignment fell 69%
   (12.54 → 3.91) and the gain moved 1.3 dB. But the remaining 4.3 dB is **not** misalignment. What
   survives alignment is genuine content difference between frames 2/24 s apart: motion blur,
   changing occlusion, independent object motion. A better flow estimator has a poor exchange rate
   from here. R-13 is downgraded and R-15 replaces it.
2. **§5.6's window policy is contradicted.** The table said low motion → K of 7–9. Measurement says
   low motion is the *only* band where multi-frame beats single-frame at all, and that **K=3 is as
   good as K=5 there**. Larger windows buy nothing and cost VRAM and time. Revised, and recorded as
   D-16.
3. **§5.8's router must gate on motion band**, not merely feed it to the window policy. Outside
   screen-anchored + slow motion, a classical multi-frame solver is measurably worse than
   single-frame.
4. **§1.4.3's oracle arm is weaker evidence than it looked.** On static clips it scores +2.66 dB by
   manufacturing phase diversity the real footage does not have. It remains the right instrument for
   "is the information there", and it is not a forecast of achievable gain.

**What this does not decide.** All of the above is measured with a *classical* solver, which treats
every aligned pixel as evidence. A learned restorer can be trained to discount poorly-corresponding
evidence. So this narrows what alignment-plus-linear-inverse can do; it does not bound what Phase 2's
model can do, and that is the open question the phase now turns on.

---

## Revision Note — what v3.2 changes and why

**The Phase 0 gate has been run.** `docs/phase0-report.md` carries the full result; the changes it
forces on this document are:

1. **§1.4.3 gains a fourth verdict, `PASS_ALIGNMENT_BLOCKED`** (D-14). The measured gate could not
   have been decided with the three the PRD originally defined, because a single multi-frame arm
   cannot separate "the information is not there" from "we could not align well enough to use it" —
   and those lead to opposite decisions.
2. **§5.7 (temporal alignment) is now the measured critical path for Phase 2**, ahead of the
   restoration model. With perfect alignment multi-frame gains **+3.30 dB** in the decision band;
   with global-translation alignment it *loses* **0.86 dB**. That gap is the entire prize.
3. **§5.4.4 (grid anchoring) is load-bearing, not diagnostic.** Object-anchored multi-frame measured
   **negative at every block size** (−0.79 to −1.50 dB): extra frames add codec noise and no
   information. A pipeline that fails to detect anchoring produces worse output than one that never
   attempted multi-frame.
4. **§5.9.4's confidence must be per-pixel.** A per-frame photometric alignment measure accepted 86%
   of neighbours in the medium-motion band and still produced −1.76 dB.
5. **§11.3's mandatory recompression now has a price tag:** roughly 23% of the recoverable gain is
   lost between CRF 18 and CRF 26.
6. **§18 R-01 is closed by measurement.** A new R-13 replaces it: alignment quality, not
   information availability, is the top technical risk.

What did **not** change: §1.4.2's predicted bands were confirmed rather than revised — the gain
peaks exactly in the 5–12 px band the document guessed at before anything was measured.

---

## Revision Note — what v3.1 changes and why

A pre-implementation review asked four control questions of v3.0. Two were already satisfied, one was
half satisfied, and one was missing. v3.1 closes the gaps; nothing else changes.

| Question | v3.0 status | v3.1 |
| --- | --- | --- |
| Are frames with no mosaic skipped? | **Yes.** Router Path C skips inference (§5.8); a file with zero detections is stream-copied rather than re-encoded (§5.1.8 R-1.8a). Detection itself necessarily runs on every frame | Clarified in §5.8; no change in behaviour |
| Can before/after comparison frames be set in settings? | **No.** Preview was interactive-only, and the temporal window `K` was preset-driven with no user override | **Added both** — comparison bookmarks (§5.16.11) and a `temporalWindow` override (§5.6.1) |
| Can restoration fail, and is it skipped? | **Partly.** Every *explicit* failure degrades to single-frame and then to pass-through (§10 E3002/E4002/E4003/E4004), and §5.14 forbids partial composites. But a restoration that *succeeds* and returns a poor result was composited regardless — confidence was diagnostic only | **Added `minRestorationConfidence`** (§5.8.1), a track-level gate with hysteresis that keeps the original pixels when the evidence is too weak |
| Is audio left alone? | **Yes.** Stream-copied, never transcoded without explicit opt-in, original timestamps, no resampling; verified by stream hash (§5.1.5, §5.1.7) | No change |

The third addition matters more than its size suggests. §1.4 says that an object-anchored grid or a
block above ~24 px leaves nothing to recover — and the engine computes exactly that as a low
confidence score, then previously composited the hallucination anyway. `minRestorationConfidence`
turns that measurement into an action, and is the second line of defence behind §5.2.5 against the
product's worst failure mode: altering footage it should have left alone.

All three settings are new keys, so they are folded into protocol v1.0 and the fingerprint sets
(§9.3) before any checkpoint exists. Adding them after implementation would have invalidated every
cached artifact (§9.3's dict-equality rule); adding them now costs nothing.
...
The gate reconstructs in **three arms** that differ only in what the neighbours are (D-14): `single`
(K=1), `oracle` (neighbours synthesised from the target at known shifts — perfect alignment), and
`estimated` (the real neighbours, aligned by the available motion model). **Only the oracle arm can
kill the product**, because only it answers whether the information exists at all.

- **PASS** — multi-frame (`K=5`) exceeds the single-frame baseline by **>= 1.0 dB PSNR** *and*
  **>= 0.03 LPIPS** *and* shows lower warping error, in **both** the oracle and estimated arms, all
  measured under the noise-floor protocol of §13.5.
- **PASS_ALIGNMENT_BLOCKED** — the oracle arm clears the thresholds but the estimated arm does not.
  The information is recoverable and the alignment model cannot reach it. **This is a pass for the
  product premise and a re-prioritisation of Phase 2**, not a stop: alignment (§5.7) becomes the
  first thing Phase 2 attacks. *(This is the verdict the gate actually returned on 2026-08-22.)*
...
(`PASS` / `PASS_ALIGNMENT_BLOCKED` / `MARGINAL` / `KILL` / `INCONCLUSIVE`) with the measured deltas
per arm and the measured noise floor, and the result is committed to `docs/phase0-report.md`.
Test: `T-GATE-P0-01`.

**Status: satisfied 2026-08-22.** Verdict `PASS_ALIGNMENT_BLOCKED`; oracle gain **+3.298 dB**,
estimated gain **−0.861 dB**, noise floor **0.000 dB**, 576 measurements. LPIPS and warping error
were **not** measured (LPIPS needs a network that arrives with Phase 2), so one of the three
criteria is satisfied and two are outstanding — the gate is re-run when they are available.
...
**Measured 2026-08-22: object-anchored multi-frame is not merely useless, it is harmful.** Even with
perfect alignment it scored **−0.79 to −1.50 dB below single-frame** at every block size, because
the extra frames contribute codec noise and no information. This makes anchoring detection
load-bearing rather than diagnostic: a pipeline that misses it produces worse output than one that
never attempted multi-frame at all.
...
| Condition | K | v3.3 measurement |
| --- | --- | --- |
| Static (< 0.25 px/frame) | **1 — multi-frame disabled** | No phase diversity; measured −0.29 dB even with the best alignment |
| Low motion (0.25-1 px/frame) | **3** | The only band that gains (+0.39 dB). K=5 measured no better than K=3 |
| Medium motion (1-6 px/frame) | **3, and only if alignment confidence is high** | −0.64 dB at K=3 with dense flow |
| High motion (> 6 px/frame) | **1 — multi-frame disabled** | −1.48 dB at K=3; correspondence is gone |
...
#### FR-6.1 User override — `temporalWindow` (new in v3.1)

The number of neighbouring frames used for reconstruction is user-settable, because it is the main
quality/throughput lever and because a user comparing output needs to be able to hold it fixed.

| Setting | Values | Default | Where |
| --- | --- | --- | --- |
| `temporalWindow` | `auto`, `3`, `5`, `7`, `9` | `auto` | Advanced (§5.16.10) |

`auto` is the adaptive policy in the table above. A fixed value replaces the **motion-based** choice
only. It does **not** override the safety reductions, all of which still apply on top of it:

- Scene-boundary truncation (§5.12) — a fixed `K=9` still truncates at a cut.
- `grid_anchor == OBJECT` → 1 (§5.4.4) — there is no phase diversity to spend frames on, so a larger
  window buys nothing but VRAM and time.
- The VRAM ladder (§5.14 step 2) — a fixed window still steps down under memory pressure rather than
  failing the job.
- Stream boundaries.

A user-forced window that ignored these would be a corruption and OOM lever rather than a quality
control. The **effective** K per frame — and the reason it differs from the requested one — is
recorded in diagnostics.

**Fingerprint:** `temporalWindow` changes the output and therefore belongs in the `restoration`
fingerprint (§9.3). It is stored as the **requested** value (`auto` stays `auto`), never as the
per-frame resolved value, which would otherwise make the fingerprint depend on scene content and
invalidate resume on every run.

**AC** — `T-WINDOW-OVERRIDE-01` asserts a fixed K is honoured in steady state.
`T-WINDOW-OVERRIDE-SAFETY-01..04` assert that each safety reduction still fires with a fixed K set.
`T-WINDOW-FINGERPRINT-01` asserts `auto` hashes stably across runs with different content.

---

### 5.7 FR-7: Temporal Alignment

> **Measured 2026-08-22: this is Phase 2's critical path.** The Phase 0 gate found **+3.298 dB**
> available in the decision band with perfect alignment and **−0.861 dB** delivered by a
> global-translation model. The gap between those two numbers is the entire benefit of the
> multi-frame architecture, and closing it is an alignment problem rather than a
> reconstruction-model problem (`docs/phase0-report.md`, §18 R-13).
...
| **C. Pass-through** | No region; region below `min_region_area`; region unconfirmed (§5.2.5b); user disabled restoration for that region; **restoration confidence below `minRestorationConfidence` (§5.8.1)**; or an explicit failure exhausted the degradation chain (§5.8.2) |

**v3.3: motion band is a gate on path A, not just an input to §5.6.** Measurement puts multi-frame's
operating window at screen-anchored grid **and** motion in roughly 0.25-6 px/frame, with the
confident part of that range being 0.25-1. Outside it a classical multi-frame solver scores below
single-frame, so the router shall select path A only inside it. `T-ROUTER-MOTION-GATE-01`.
...
#### FR-8.1 Confidence gate — `minRestorationConfidence` (new in v3.1)

v3.0 handled every way restoration could *fail*, and no way it could *succeed badly*. §1.4 says an
object-anchored grid or a block above ~24 px leaves nothing to recover; §5.9.4 computes exactly that
as a low confidence score; and the pipeline then composited the result anyway. This requirement turns
the measurement into an action.

| Setting | Range | Default | Where |
| --- | --- | --- | --- |
| `minRestorationConfidence` | `0.00 - 0.90` | **`0.00` (off)** | Advanced (§5.16.10) |

- **R-8.1a** When the gate is active and a region's restoration confidence falls below it, the
  restored ROI is **discarded** and the **original pixels are kept unmodified**. Nothing is blended,
  so there is no seam to hide: the region is simply untouched.
- **R-8.1b** The default is `0.00`, which reproduces v3.0 behaviour exactly. This is a switch the user
  opts into, not a silent change of what the product does.
- **R-8.1c** **The decision is made per track with hysteresis, never independently per frame.** A raw
  per-frame threshold applied to a marginal region would flip between restored and original from
  frame to frame — which looks far worse than either choice held consistently, and would land
  precisely where confidence is borderline. Enter the gated state when the track's smoothed
  confidence stays below `minRestorationConfidence` for `>= 3` frames; leave it when the smoothed
  confidence exceeds `minRestorationConfidence + 0.05` for `>= 3` frames.
- **R-8.1d** Gated regions are counted in the job summary and reported to the user
  ("N regions left untouched: evidence below threshold"). Silently declining to do the job the user
  asked for is worse than doing it badly.
- **R-8.1e** Transitions between gated and ungated state are alpha-ramped over the hysteresis window
  by the existing temporal alpha smoothing (§5.11), so the switch is not a visible pop.

**Fingerprint:** in the `restoration` set (§9.3) — it changes output.

**AC** — `T-CONF-GATE-01` asserts a below-threshold region is byte-identical to the source.
`T-CONF-GATE-HYSTERESIS-01` asserts a confidence signal oscillating around the threshold produces at
most one state change, not one per frame. `T-CONF-GATE-DEFAULT-01` asserts the default of `0.00`
gates nothing. `T-CONF-GATE-REPORT-01` asserts gated regions appear in the summary.

#### FR-8.2 Failure degradation chain (restated for clarity, no behaviour change)

Every explicit failure degrades rather than failing the job, and the chain terminates in untouched
source pixels:

```text
multi-frame  --E4003 alignment failed-->  single-frame
single-frame --E4002 inference failed, retry once--> pass-through (original pixels)
any          --E4004 ROI below model minimum-->      pass-through
detection    --E3002 detector failed, retry once-->  pass-through for that frame
any          --GPU OOM-->  §5.14 ladder --exhausted--> E4401, job fails, output discarded
```

**A frame is either fully restored or bit-identical to its source. There is no third state.** §5.14's
no-partial-composite rule (`T-VRAM-NO-PARTIAL-01`) is what enforces it on the OOM path, and
`T-DEGRADE-CHAIN-01..04` on the others.
...
**v3.2, measured:** alignment confidence shall be derived **per pixel** from dense flow, not per
frame from a photometric residual. In the Phase 0 gate a per-frame residual-ratio filter admitted
86% of neighbours in the medium-motion band and the result was still −1.76 dB: the measure accepts
frames whose *global* alignment is plausible while their *local* motion is not.

**v3.1:** confidence is no longer purely diagnostic. It drives the §5.8.1 gate when the user enables
one, which raises the bar on this number: a confidence score that is poorly calibrated now
*withholds restoration* rather than merely mislabelling it. Phase 2 shall therefore report a
**calibration curve** (predicted confidence vs. measured PSNR/LPIPS improvement over pass-through on
`eval-v1`) alongside the monotonicity check, so that a threshold like `0.4` means something the user
can reason about. `T-CONF-CALIBRATION-01` is a reported experiment, not a pass/fail gate.
...
Interactive preview alone is not enough for judging quality across runs — see §5.16.11 for persisted
comparison points.
...
#### 5.16.10 Advanced settings panel (new in v3.1)

Everything not in a preset lives here, with its default, its range, and — for anything that changes
output — a note that changing it invalidates cached work (§9.3).

| Setting | Range | Default | Fingerprint | Section |
| --- | --- | --- | --- | --- |
| Detection confidence | `0.10 - 0.90` | `0.45` | detection | §5.2.3 |
| NMS IoU | `0.30 - 0.80` | `0.50` | detection | §5.2.3 |
| `min_region_area` | `64 - 4096 px²` | `256 px²` | detection | §5.2.3 |
| Mask binarization threshold | `0.30 - 0.70` | `0.50` | detection | §5.2.3 |
| `min_confirm_frames` | `1 - 5` | `2` | detection | §5.2.5b |
| `max_missing_frames` | `0 - 15` | `3` | detection | §5.3.4 |
| `padding_ratio` | `0.10 - 0.20` | `0.15` | restoration | §5.5.1 |
| **`temporalWindow`** | `auto, 3, 5, 7, 9` | **`auto`** | restoration | **§5.6.1** |
| `align_conf_min` | `0.0 - 0.9` | `0.35` | restoration | §5.7 |
| **`minRestorationConfidence`** | `0.00 - 0.90` | **`0.00` (off)** | restoration | **§5.8.1** |
| Feather width | `1 - 9 px` | `3 px` | restoration | §5.11 |
| Encoder profile | `Quality (x265) / Speed (NVENC)` | `Quality` | encode | §5.1.4 |
| VRAM budget | `Auto, 4, 6, 8, 12+ GB` | `Auto` | **none** — performance knob | §5.14 |
| Precision | `Auto, fp16, fp32` | `Auto` | **none** — performance knob, and the OOM ladder changes it at runtime | §5.14, §9.3 |
| **Comparison points** | list of timecodes | empty | **none** — diagnostic only | **§5.16.11** |

**AC** — `T-SETTINGS-FINGERPRINT-MAP-01` asserts every setting is assigned to exactly one fingerprint
set or explicitly excluded, and that the excluded ones are excluded for the reason recorded in §9.3.
A setting added without an entry here fails the test.

#### 5.16.11 Comparison points (new in v3.1)

Interactive scrubbing answers "does this frame look right"; it does not answer "is this run better
than the last one", because the two runs are never looked at from the same position. Comparison
points fix that.

- The user marks any number of timecodes on the job (from the preview, or by typing them). They are
  stored with the job (§9.2) and survive restart, retry, and settings changes.
- At the end of processing, the engine renders **original / restored PNG pairs** at each marked
  timecode into `preview/compare/<pts>.{orig,rest}.png` (§8.6) and the UI presents them side by side.
- Because the points are persisted and the renders are deterministic given the same output, two runs
  with different settings produce directly comparable image pairs at identical positions. That is the
  minimum needed to judge a setting change by eye rather than by impression.
- Comparison points **do not enter any fingerprint** — they change nothing about the output. Same rule
  and same reason as the performance knobs in §9.3.
- Default: empty. When empty, nothing is rendered and nothing is slower.

**AC** — `T-COMPARE-POINTS-PERSIST-01` asserts the points survive a job restart.
`T-COMPARE-POINTS-RENDER-01` asserts one image pair per point at the exact requested PTS.
`T-COMPARE-POINTS-NOFP-01` asserts adding or removing points discards no cached artifact.
...
| `process` | Full pipeline | `jobId`, `sourcePath`, `outputPath`, `settings`, `resume`, `comparisonPts[]` (§5.16.11) |
...
```jsonc
{
  ...
  "confidence": {
    "mean": 0.41,
    "bucketCounts": { "high": 120, "medium": 900, "low": 300 },
    "gatedRegions": 0                      // left untouched by §5.8.1; reported to the user
  },
  "comparisonPoints": [ 61250, 1840500 ]   // §5.16.11; PTS in the source time base. Not fingerprinted
}
```
...
- **Performance knobs are excluded from every fingerprint**: VRAM budget, tile size, batch size,
  ...
  completed work.
- **Diagnostic knobs are excluded for the same reason, stated differently**: they do not affect output
  pixels. Currently: comparison points (§5.16.11) and diagnostic-overlay toggles. Adding or moving a
  comparison point shall never discard a completed restoration — if it does, the user cannot inspect
  a finished job without paying to re-run it.
- **New in v3.1, both in the `restoration` set**: `temporalWindow` (§5.6.1) and
  `minRestorationConfidence` (§5.8.1). Both change output pixels, so both belong there. `temporalWindow`
  is hashed as the **requested** value — `auto` stays `auto` — never as the per-frame resolved K, which
  depends on scene content and would make the fingerprint unstable across runs of the same job.
...
| W4102 | Region left untouched: confidence below `minRestorationConfidence` (§5.8.1) | — |
| W4103 | Requested `temporalWindow` reduced by a safety rule (carries the rule and the effective K, §5.6.1) | — |
...
| Split | Content | Scale |
| --- | --- | --- |
| `clean-train` | Licensed/permissive clean video: varied resolution, grain, motion, lighting, skin tones, fabrics, text, faces at varying scale | **measure, do not guess** |
| `clean-val` | Same distribution, **disjoint sources** | as above |
| `clean-test` | Same distribution, disjoint sources, frozen | as above |
| `negatives` | §11.4 | the binding constraint — see below |
| `real-mosaic` | Real-world mosaicked clips, **unpaired**, for domain-gap checking and detector recall only | small; it is a check, not training data |

**Corrected in v3.2.** Earlier revisions of this document put concrete hour targets here (40 h / 4 h
/ 6 h / 2 h). **Those numbers were never derived from anything** — they were plausible-sounding
figures written before any experiment existed, which is exactly what §12.7 forbids elsewhere in this
document. They are withdrawn.

What replaces them is a rule and an escalation:

- **Diversity, not hours, is what the detector needs.** The positive class is synthetic and
  therefore unlimited: the generator produces as many mosaics as wanted, with perfect masks. What
  cannot be manufactured is the variety of *clean* content the detector must not fire on.
- **Escalate on measurement.** Train on what exists, measure held-out mask IoU, and add data only
  when generalisation — not intuition — says the corpus is the limit.
  1. Can a detector learn the signature at all? Answer it with the corpus already in hand. A failure
     here is an architecture problem (D-03), and no quantity of video fixes it.
  2. If yes, expand **negatives** (§11.4) next. The false-positive requirement of §5.2.5a is the
     binding constraint, and no amount of additional *clean* footage improves it.
  3. Expand clean sources only when measurement shows cross-content generalisation failing. CC-BY
     open films are cheap to add; being cheap is not a reason to add them before they are the limit.

**AC-11.2** — every hour figure in a dataset plan cites the measurement that produced it, or is
marked as an estimate. Test: none; this is a review rule.
...
**Roughly half of these can be manufactured authentically from clean footage, and should be**
(v3.2). Re-encoding a clean clip at a very low bitrate produces *real* encoder blocking, not a
simulation of it; downscale-then-upscale produces real resampling softness; grain can be added.
Those categories cost nothing to obtain and belong in the generator alongside the positives.

**What cannot be manufactured must be collected:** defocus and bokeh are optical, and content that
is *genuinely* on a grid — pixel art, LED walls, mesh fabric, blinds — has to be real, because it is
precisely the case where a synthetic approximation would teach the detector the wrong boundary.
...
**Entry:** Phase 1 exit.
**Work, in the priority order the Phase 0 measurement sets:**

1. **Alignment (§5.7) first.** It is the critical path: +3.30 dB is available and the current model
   delivers −0.86 dB. Dense optical flow, **per-pixel** confidence, occlusion handling.
2. **Grid-anchoring detection reliable enough to gate on** (§5.4.4). Object-anchored multi-frame is
   measurably harmful, so the fallback must work before multi-frame ships.
3. Then the degradation-conditioned model (D-04), router (§5.8), temporal consistency (§5.10),
   confidence and its calibration (§5.9.4), the conditioning ablation (§5.9.3), **and the
   initialization comparison.**

**Re-run the Phase 0 gate once dense alignment exists.** The `estimated` arm is the number that has
to move, and it is the honest measure of whether Phase 2 succeeded.
...
| 14 | Before/after preview including split view, **and persisted comparison points rendered at job end** | `T-UI-PREVIEW-COALESCE-01`, `T-COMPARE-POINTS-RENDER-01` |
| 14b | **Every frame is either fully restored or bit-identical to source — never partial** | `T-VRAM-NO-PARTIAL-01`, `T-DEGRADE-CHAIN-01..04` |
...
| --- | --- | --- | --- | --- | --- |
| R-01 | ~~Multi-frame gives no real benefit on representative material~~ | — | — | **Closed by measurement 2026-08-22.** Oracle gain +3.298 dB in the decision band, well clear of the 1.0 dB threshold and a 0.000 dB noise floor. Re-open if LPIPS or warping error contradict it | — |
| R-13 | ~~Alignment cannot reach the available information~~ | Medium (was High) | — | **Partly closed 2026-08-22.** Dense flow cut the post-alignment residual by 69% and recovered 1.3 dB of a 5.6 dB gap. Further flow work has a poor exchange rate (`docs/phase2-alignment-report.md` §4.1) | Eng |
| **R-15** | **Neighbouring frames do not contain the target's content**, and no alignment can change that. Measured: 19% of the gap is alignment, the rest is correspondence | **High — it bounds multi-frame's operating window to screen-anchored + slow motion** | **High, measured** | Router gates on motion band (§5.8); K defaults to 3 (§5.6); **the open question is whether a learned restorer tolerates what a classical solver cannot** — that is Phase 2's real experiment | ML |
| **R-14** | **Grid-anchoring detection misses, and object-anchored regions get fused** | Medium — measurably *worse* than not restoring (−0.79 to −1.50 dB) | Medium | §5.4.4 estimator gated in Phase 1; §5.8 router fallback; `T-PROFILE-ANCHOR-01..03` | ML |
...
| 26 | **Restoration succeeds but the result is unsupported by evidence** (v3.1) | §5.8.1 gate keeps original pixels, `W4102`, counted in the summary | `T-CONF-GATE-01` |
| 27 | **Confidence oscillates around the gate threshold** (v3.1) | Track-level hysteresis, alpha-ramped transition — not a per-frame flip | `T-CONF-GATE-HYSTERESIS-01` |
| 28 | **User forces `temporalWindow=9` on object-anchored or scene-cut content** (v3.1) | Safety reductions still apply; effective K and reason reported via `W4103` | `T-WINDOW-OVERRIDE-SAFETY-01..04` |
