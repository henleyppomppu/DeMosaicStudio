...
**Primary Acceleration:** NVIDIA CUDA (PyTorch / ONNX Runtime / TensorRT), NVDEC / NVENC
**Fallback Acceleration:** ONNX Runtime DirectML / CPU
**Document Status:** Implementation-Ready Draft
**Version:** 2.0 (supersedes 1.0; the original is preserved verbatim at `prd.v1.0-original.md`)
...
## Revision Note — what v2.0 changes and why
...
| G-04 | No license audit | Ultralytics YOLO is AGPL-3.0; BasicVSR++ is S-Lab non-commercial. Both were named as primary candidates | §4.2 |
...
| G-12 | No model packaging, versioning, download, or verification plan | Weights are hundreds of MB to GB; the installer story and the first-run story are undefined | §14 |
...
| G-15 | DirectML fallback promised for models that may use custom or deformable ops | The fallback may not run the model at all; promising it creates a support burden | §5.17, §18 R-07 |
...
| D-03 | Detector = **binary semantic segmentation**, U-Net decoder over a `timm` encoder (Apache-2.0), trained in-house | Ultralytics YOLOv8-seg (AGPL-3.0) | Medium. An encoder/decoder swap is a training run, not an architecture change; the license constraint is non-negotiable |
| D-04 | Restoration = **in-house recurrent degradation-conditioned model**, trained on the synthetic generator's degradation | Adopt BasicVSR++ weights (non-commercial license, and trained for bicubic SR rather than pixelation) | High. This is most of Phase 2. See §5.9 and §18 R-02 |
...
| D-09 | Inference runtime for MVP is **PyTorch + CUDA**; ONNX export and TensorRT are a Phase 5 optimization gated on numerical-parity tests | TensorRT from day one | Low, if and only if the model uses export-safe ops (§5.17) |
...
- Local Windows processing, NVIDIA GPU acceleration, DirectML/CPU fallback.
...
- **C-1** The application is for restoring video the user owns or is otherwise authorized to
  process. The installer and first-run screen shall state this.
- **C-2** No model shipped with the product shall be trained on, or fine-tuned toward reconstructing,
...
**AC-2.3** — A release-blocking checklist item verifies C-1 through C-6 against the built installer
and the shipped string tables. Test: `T-POLICY-STRINGS-01` scans `Strings.resx` for the banned
vocabulary of C-3.
...
3. An embedded Python runtime enlarges the installer and complicates packaging (§14.4).
...
| Engine | Python | 3.12.x, embedded distribution | §14.4 |
| Tensor runtime | PyTorch + CUDA | torch 2.x + cu12x wheel | D-09 |
| Encoder/decoder | FFmpeg via PyAV | FFmpeg 7.x LGPL build | §4.2 |
...
| Export (Phase 5) | ONNX Runtime, TensorRT | pinned per release (§6.3) | D-09 |
| Installer | Inno Setup 6 | | §14.4 |
| Native engine (Phase 5+) | C++20, CMake, vcpkg | deferred | §3.4 |

### 4.2 License audit — **blocking**

v1.0 named two technologies on its front page that cannot ship in this product.

| Component | License | Verdict |
| --- | --- | --- |
| **Ultralytics YOLOv8/v11 (incl. `-seg`)** | **AGPL-3.0** | **Rejected.** AGPL obliges source disclosure for a distributed desktop application. A commercial Ultralytics license is the only alternative, and buying one to get a detector we must fine-tune anyway is poor value. → D-03 |
| **BasicVSR++ / BasicSR model zoo weights** | **S-Lab License 1.0 — non-commercial only** | **Rejected for shipping.** May be used for *internal Phase 0/2 baselines only*, never redistributed and never in a shipped build. → D-04 |
| FFmpeg | LGPL-2.1+ (GPL if built with `--enable-gpl`) | **Allowed with care.** Ship an **LGPL** build, dynamically linked, with written offer and notices. Do **not** enable x264/x265 (GPL) in the shipped build; use NVENC and, for software fallback, an LGPL-compatible encoder or the OS encoder. If x264/x265 is required, the product becomes GPL — escalate before assuming |
| PyTorch | BSD-3 | Allowed |
| `timm` | Apache-2.0 | Allowed. Verify each pretrained checkpoint's own license separately |
| ONNX Runtime / DirectML | MIT | Allowed |
| TensorRT | NVIDIA SLA | Allowed, redistribution terms apply; pin version per release |
| ByteTrack (reference impl) | MIT | Allowed; the algorithm is reimplemented anyway to avoid dependency weight |
| LPIPS | BSD-2, weights from VGG/AlexNet (varied) | **Evaluation only.** Not shipped |
| Any pretrained encoder checkpoint | varies | **Each checkpoint requires its own line in `THIRD_PARTY_NOTICES.md` before it enters the repo** |

**AC-4.2** — `THIRD_PARTY_NOTICES.md` exists, lists every shipped dependency with license and source
URL, and CI fails if a dependency appears in a lockfile without a corresponding notice entry. Test:
...
DeMosaicStudio/
  ...
  worker/
    ...
    requirements.lock
    tests/
  training/                        # NOT shipped. datasets, degradation generator, train, eval
    degradation/ datasets/ models/ train_detector.py train_restorer.py
  scripts/
    build-worker.ps1 build-installer.ps1 smoke-gpu.ps1
    eval_multiframe_gate.py eval_report.py
  tests/
...
- CI runs on Linux with `EnableWindowsTargeting=true`, which compiles WPF without running it. This
  catches XAML and binding-signature breakage on every push.
- CI runs the **GPU-free subset** (§13.2). GPU-dependent tests are marked and skipped there, and are
  covered by `scripts/smoke-gpu.ps1` on the dev machine.
- **Baseline discipline:** platform-dependent tests will fail on one OS or the other. Record the
  ...
  regression.
- Scripts (`.ps1`) and installer scripts (`.iss`) shall be saved as **UTF-8 with BOM**. Windows
...
fails on any new script that lacks one.
...
Priority: NVENC → software. User-selectable output codec: H.264, H.265. Default rate control:
constant-quality targeting visual transparency on untouched regions (see FR-1.8), not a fixed
bitrate.
**AC** — Encoder selection is observable in the result payload and the produced file's stream
metadata matches the request. `T-IO-ENCODE-SELECT-01`.
...
- **R-1.8b** Otherwise the default is a full re-encode at a quality-preserving target: CQ/CRF chosen
  ...
  **>= 42 dB PSNR / >= 0.99 SSIM** against the source. That figure defines "visually transparent" for
  this product, and the default CQ is derived from it per codec, not guessed.
...
**AC** — The null-run test asserts the transparency threshold for each shipped codec/preset, and the
...
Constraint carried over from v1.0 and made testable: any chosen implementation shall either be
natively supported by every shipped backend, or ship with a tested fallback for backends that lack
it. This is the single most likely source of a "works in PyTorch, fails in ONNX/TensorRT/DirectML"
break, so the op-support gate of §5.17 applies to it first.
...
Three shipped implementations, one per quality preset (§15): `Fast`, `Balanced`, `Quality`. The router
...
Third-party VSR weights may be used as **internal Phase 0/2 baselines only** and are never shipped
(§4.2).
...
`Auto` (default), `NVIDIA CUDA`, `DirectML`, `CPU`. Unavailable options are disabled **with the reason
shown**, not hidden. A precision explicitly saved as `float16` on an NVIDIA machine shall not be
...
- **R-17a** Every model shipped shall pass an **op-support gate** before a backend is advertised for
  it: the model exports cleanly and produces numerically equivalent output (per §13.6 tolerances) on
  that backend. A backend that fails the gate for a given model is **not offered** for that model.
- **R-17b** DirectML support is therefore *per model*, not global. If the restoration model uses an op
  DirectML cannot run, the product offers DirectML for detection only and states that restoration
  requires CUDA or CPU — rather than offering it and failing at load.
- **R-17c** Non-NVIDIA GPUs (AMD, Intel Arc) have no CUDA path. Where DirectML fails the gate, the
  product runs on CPU and **says so before the job starts**, with an honest speed estimate. Silent
  10x-slower operation is a support ticket; a stated estimate is a choice.
- **R-17d** TensorRT engines are not portable across GPU architectures or TensorRT versions. Engines
  are built or cached on first use per (GPU, driver, TRT version), with a visible one-time
  "optimizing for your GPU" step and a cache under the app data directory.

**AC** — `T-BACKEND-GATE-01` asserts an unsupported (model, backend) pair is not selectable in the UI
and cannot be forced through settings.
---
...
Two tiers, because one number that no shipping build can meet is not a target (D-01, §3.4):

| Component | MVP target (Python engine) | Optimized target (Phase 5) |
| --- | ---: | ---: |
| Detector only | >= 60 FPS | >= 120 FPS |
| End-to-end, restoration active | **>= 8 FPS** | >= 15 FPS |
...
reordering, model initialization, TensorRT engine build (§5.17d), and temporal look-ahead all vary by
...
- Windows 10 x64 build 19041+, Windows 11 x64.
- NVIDIA GPUs supported by the CUDA/TensorRT versions pinned **per release** in
  `docs/RUNTIME_MATRIX.md`. The product does not claim "CUDA 12.x" in general.
- Non-NVIDIA: DirectML per §5.17b; otherwise CPU.
...
Every source file records its license in `training/datasets/SOURCES.md`. A source without a recorded
license does not enter the corpus.
...
### 13.6 Numerical-parity tolerances (for §5.17 backend gating)

PyTorch FP32 is the reference. A backend passes the gate for a model when, on the gate fixture set:
mean absolute error <= `1e-3` and max absolute error <= `1e-2` in `[0,1]` pixel space, and the §12.3
metrics differ from the reference by less than the §13.5 noise floor.
...
## 14. Model Packaging and Distribution (closes G-12)

### 14.1 Layout and manifest

```text
models/
  manifest.json              # signed; lists every model, version, sha256, size, url
  detector/
    det-unet-b0-1.2.0/
      model.pt | model.onnx
      metadata.json
...
normalization), supported temporal windows, precision, supported runtimes (per §5.17 gating),
required preprocessing, output semantics, training dataset version, and license.

### 14.2 Distribution

- Models are **not** bundled in the installer beyond one small default detector; the rest download on
  first use.
- Downloads are resumable, verified by SHA-256 against `manifest.json`, and written atomically.
- **Do not hard-code remote file lists.** Enumerate the release's file list from the source of truth
  and select by pattern. Hard-coded filenames rot: names change, ids get typos, and large weights get
  split into multiple parts. Static lists are acceptable only as an **offline fallback**, and a test
  shall check the static list against the live listing.

### 14.3 Missing-model handling before a job starts

If the resolved settings require a model that is not installed, the host shall ask **before** the job
starts, show the download size, and download on confirmation. It shall not start a job that will fail
after minutes of decoding.

Critically, **the resolution result must reach execution**: if `auto` resolves to `rest-quality` and
the host then sends `auto` to the worker, the worker applies its own hard-coded default and the user
gets a different model than the one they approved. The resolved snapshot — not the stored settings
object — is what is sent. The snapshot is a copy; resolving models does not mutate the user's saved
settings.

If hardware detection or model-status lookup fails, proceed **without** the confirmation dialog rather
than blocking the queue; the worker still reports the problem, later.

**AC-14.3** — `T-MODEL-RESOLVE-SNAPSHOT-01` asserts the worker receives a concrete model id, never
`auto`, whenever resolution succeeded. `T-MODEL-CONFIRM-01` asserts the dialog appears with the
correct total size and that declining leaves the queue untouched.

### 14.4 Python runtime packaging

The worker ships as an embedded Python distribution under `tools/python/`, populated by
`scripts/build-worker.ps1` (pinned wheels from `worker/requirements.lock`). The installer bundles it.
The runtime is app-scoped, not a system Python. `tools/` is git-ignored; the build script is the
source of truth for what goes in it.

**AC-14.4** — A clean-machine install runs a full job with no system Python, no system FFmpeg, and no
CUDA Toolkit present. `T-INSTALL-CLEANROOM-01` (manual, release checklist).
...
**Entry:** none.
**Work:** `git init`; repo skeleton (§4.3); `scripts/check-environment.ps1`; embedded Python + FFmpeg;
...
**Entry:** Phase 1 exit.
**Work:** train the in-house degradation-conditioned model (D-04); alignment (§5.7); router (§5.8);
temporal consistency (§5.10); confidence (§5.9.4); the conditioning ablation (§5.9.3).
**Exit:** on `eval-v1`, multi-frame beats single-frame per band per §12.3, temporal AC of §5.10 met,
confidence monotonicity AC met.
Note: v1.0's 2-4 week estimate assumed adopting a pretrained model. D-04 makes this the longest phase.
...
**Entry:** Phase 4 exit.
**Work:** ONNX/TensorRT export with the §13.6 parity gate, tiling, adaptive window tuning, GPU-resident
decode/encode, and — if justified by measurement — the native engine (§3.4).
**Exit:** §6.1 Optimized targets met on the reference config, with ROI coverage reported.

### Phase 6 — Packaging and QA (1-2 weeks)

**Entry:** Phase 5 exit.
**Work:** installer, crash recovery, long-run tests, `THIRD_PARTY_NOTICES.md` completion, the §22
acceptance matrix, clean-room install test.
**Exit:** release candidate; every AC in this document has a passing or explicitly waived test.
...
| 10 | Export H.264/H.265; untouched regions visually transparent | `T-QUALITY-NULLRUN-01` |
...
| R-05 | **License contamination** (AGPL/non-commercial weights) | Existential-legal | Medium (it was already in v1.0's stack) | §4.2 audit, notice-coverage CI gate, no third-party weights in shipped builds | Tech lead |
| R-06 | **Python engine too slow to be pleasant** | Medium | Medium | Two-tier targets (§6.1); Phase 5 native path reserved; boundary designed for swap (§3.4) | Eng |
| R-07 | **Backend fallback (DirectML) cannot run the model** | Medium | High | Per-model op-support gate (§5.17); do not advertise what does not pass | Eng |
| R-08 | **TensorRT engine portability / first-run build time** | Low-Medium | High | Per-GPU cache with visible one-time step (§5.17d) | Eng |
...
Answer these before Phase 1; Phase 0 can start without them.

| # | Question | Default if unanswered |
| --- | --- | --- |
| Q1 | **Do you accept D-01 (Python worker for MVP, native engine deferred to Phase 5)?** To override, replace D-01 with the v1.0 native-first plan and add ~3-4 weeks of Phase 0 toolchain and ABI work | Proceed with D-01 |
| Q2 | Is a commercial/closed distribution intended? If the product will be open-sourced under AGPL-compatible terms, the §4.2 rejections of Ultralytics and BasicVSR++ can be revisited | Assume closed, commercial |
| Q3 | Which clean-video sources are licensed for training? R-03 and §11.2 depend on it | Blocked; Phase 1 cannot start |
| Q4 | Is batch/folder processing needed for v1, or is single-job acceptable? (§2.2) | Single job |
| Q5 | Is 4K a v1 requirement or a v2 one? It changes tiling and VRAM work materially | v1 supports 4K at reduced throughput; not a performance target |
| Q6 | Localization scope beyond English? | English only, strings externalized |
| Q7 | Is a "detect and report only" mode (no restoration) worth shipping as a fallback if R-01 fires? | Yes — it is the re-scope target in §1.4.3 |
...
| Axis | Values |
| --- | --- |
| Hardware | RTX 3080 Ti 12 GB (primary dev), RTX 3060-class, higher-VRAM RTX, DirectML-capable non-NVIDIA, CPU-only |
