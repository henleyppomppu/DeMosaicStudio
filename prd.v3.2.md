...
**Version:** 3.2 — Phase 0 gate measured; alignment identified as the critical path (supersedes 3.1)
**Distribution:** **Personal use only. This software is not distributed to anyone.** See §2.4 and D-11.
**Version history:** `prd.v1.0-original.md` (original), `prd.v2.0.md` (implementation-ready), `prd.v3.0.md` (personal-use scope), `prd.v3.1.md` (control gaps closed)
...
| **R-13** | **Alignment cannot reach the available information** (§5.7). The gate measured +3.30 dB available and −0.86 dB delivered | **High — it is the entire multi-frame benefit** | **High, measured** | Dense flow with per-pixel confidence as Phase 2's first work item; re-run the gate to measure the `estimated` arm; §5.8 falls back to single-frame whenever alignment is poor, so the failure mode is "no benefit", never "damaged output" | Eng |
