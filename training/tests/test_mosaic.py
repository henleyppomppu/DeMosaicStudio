...
def test_a_screen_anchored_grid_gives_phase_diversity_as_the_subject_moves() -> None:
    ...
    spec = MosaicSpec(block_width=8, block_height=8, anchor=GridAnchor.SCREEN)
...
def test_an_object_anchored_grid_has_no_phase_diversity() -> None:
    ...
    spec = MosaicSpec(block_width=8, block_height=8, anchor=GridAnchor.OBJECT)
...
def test_phase_diversity_is_zero_for_an_empty_sequence() -> None:
    assert phase_diversity(MosaicSpec(), []) == 0.0


def test_the_grid_offset_shifts_the_block_boundaries() -> None:
    image = _ramp()
    aligned = pixelate(image, MosaicSpec(block_width=8, block_height=8, grid_offset_x=0))
    shifted = pixelate(image, MosaicSpec(block_width=8, block_height=8, grid_offset_x=4))
...
def test_non_square_blocks_are_supported() -> None:
    ...
    out = pixelate(_ramp(), MosaicSpec(block_width=16, block_height=4))
...
def test_opacity_blends_towards_the_source() -> None:
    image = _ramp()
    opaque = pixelate(image, MosaicSpec(block_width=8, block_height=8))
    half = pixelate(image, MosaicSpec(block_width=8, block_height=8, opacity=0.5))
...
def test_generation_is_deterministic_given_a_seed() -> None:
    ...
    base = MosaicSpec(kind=DegradationType.PIXELATION)
...
def test_random_geometry_keeps_anchoring_and_type() -> None:
    base = MosaicSpec(kind=DegradationType.BOX_BLUR, anchor=GridAnchor.OBJECT)
...
def test_invalid_specs_are_rejected() -> None:
    with pytest.raises(ValueError):
        MosaicSpec(block_width=0)
    with pytest.raises(ValueError):
        MosaicSpec(opacity=1.5)
